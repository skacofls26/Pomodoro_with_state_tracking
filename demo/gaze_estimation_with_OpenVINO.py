...
def main():
    core = Core()
    models = {
        "face": load_model(core, f"{model_dir}/face-detection-adas-0001/FP32/face-detection-adas-0001"),
        "head_pose": load_model(core, f"{model_dir}/head-pose-estimation-adas-0001/FP32/head-pose-estimation-adas-0001"),
        "landmarks": load_model(core, f"{model_dir}/facial-landmarks-35-adas-0002/FP32/facial-landmarks-35-adas-0002"),
        "gaze": load_model(core, f"{model_dir}/gaze-estimation-adas-0002/FP32/gaze-estimation-adas-0002"),
    }
    ...
    while True:
        ...
        for det in detections[0][0]:
            ...
            Lin_x, Lin_y = int(landmarks[0] * face.shape[1]), int(landmarks[1] * face.shape[0]) 
            Lout_x, Lout_y = int(landmarks[2] * face.shape[1]), int(landmarks[3] * face.shape[0]) 
            Rin_x, Rin_y = int(landmarks[4] * face.shape[1]), int(landmarks[5] * face.shape[0]) 
            Rout_x, Rout_y = int(landmarks[6] * face.shape[1]), int(landmarks[7] * face.shape[0]) 
            ...
            focused = abs(theta) < 0.35 and -0.43 < phi < 0.12
